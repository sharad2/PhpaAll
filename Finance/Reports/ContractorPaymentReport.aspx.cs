    ...
    public partial class ContractorPaymentReport : PageBase
    {
        ...
        private IEnumerable<ContractorRecoverdPayment> QueryIterator(IQueryable<RoVoucher> query, int jobId)
        {
            foreach (RoVoucher v in query)
            {
                ...
                //                            && p.JobId == jobId)
                //                            ? p.CreditAmount ?? 0 : 0));
                crp.MaterialRecoverd = (decimal?)v.RoVoucherDetails.Sum(p => ((HeadOfAccountHelpers.AdvanceSubTypes.MaterialAdvance.Contains(p.RoHeadHierarchy.HeadOfAccountType)
                                            && p.JobId == jobId)
                                            ? p.CreditAmount ?? 0 : 0));

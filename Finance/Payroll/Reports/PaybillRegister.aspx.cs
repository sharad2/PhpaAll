        ...
        protected void dsEmployee_Selecting(object sender, LinqDataSourceSelectEventArgs e)
        {
            ...
            e.Result =
                  ...
                  select new
                  {
                      ...
                      BankAccountNo = (g.Select(p => p.empp.BankAccountNo).Distinct().Count() == 1 && g.Max(p => p.empp.BankAccountNo) != null) ? g.Max(p => p.empp.BankAccountNo) : g.Key.Employee.BankAccountNo,
                      BankName = (g.Select(p => p.empp.Bank.BankName).Distinct().Count() == 1 && g.Max(p => p.empp.Bank.BankName) != null) ? g.Max(p => p.empp.Bank.BankName) : g.Key.Employee.Bank.BankName,

        ...
        protected void dsEmployee_Selecting(object sender, LinqDataSourceSelectEventArgs e)
        {
            ...
            e.Result =
                  ...
                  select new
                  {
                      ...
                      BankAccountNo =  g.Select(p => p.empp.BankAccountNo).Distinct().Count() > 1 ? g.Key.Employee.BankAccountNo : g.Max(p => p.empp.BankAccountNo),
                      BankName = g.Select(p => p.empp.Bank.BankName).Distinct().Count() > 1 ? g.Key.Employee.Bank.BankName : g.Max(p => p.empp.Bank.BankName),
